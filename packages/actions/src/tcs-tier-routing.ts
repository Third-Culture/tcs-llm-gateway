...
export interface TcsTierVerifiedPricing {
	providerId: string;
	modelName: string;
...
export interface TcsTierRoutingStatusEntry {
	...
	verifiedPricing: TcsTierVerifiedPricing | null;
	primaryPricing: TcsTierVerifiedPricing | null;
}
...
function toVerifiedPricing(
	provider: TcsTierProviderMapping | undefined,
): TcsTierVerifiedPricing | null {
	if (!provider) {
		return null;
	}
	return {
		...
		inputPricePerMillion: provider.inputPrice * 1e6,
		outputPricePerMillion: provider.outputPrice * 1e6,
	};
}

function findProviderPricing(
	providers: TcsTierProviderMapping[],
	providerId: string | null | undefined,
	modelName: string | null | undefined,
): TcsTierProviderMapping | undefined {
	if (!providerId) {
		return undefined;
	}
	if (modelName) {
		const exact = providers.find(
			(provider) =>
				provider.providerId === providerId && provider.modelName === modelName,
		);
		if (exact) {
			return exact;
		}
...
export async function runTcsTierRoutingChecks(): Promise<TcsTierRoutingRunSummary> {
	...
	for (const tier of tiers) {
		...
		results.push(result);
		const fallbackProviders = tier.providers.slice(1).map((provider) => ({
			providerId: provider.providerId,
			modelName: provider.modelName,
		}));
		await db
...
export async function getTcsTierRoutingStatusSnapshot(): Promise<TcsTierRoutingStatusSnapshot> {
	...
	const responseTiers = tiers.map((tier) => {
		...
		const tierStatus =
			status?.status === "ok"
				...
				: status?.status === "error"
					? "error"
					: "unknown";
		const primaryPricing = toVerifiedPricing(tier.providers[0]);
		const selectedPricing = toVerifiedPricing(
			findProviderPricing(
				tier.providers,
		...
		return {
			...
			status: tierStatus as "ok" | "error" | "unknown",
			selectedProvider: status?.selectedProvider ?? null,
			selectedModel: status?.selectedModel ?? null,
			primaryProvider: tier.primaryProvider,
			primaryModel: tier.primaryModel,
			fallbackProviders: tier.providers.slice(1).map((provider) => ({
				providerId: provider.providerId,
				modelName: provider.modelName,
			})),
			...
			verifiedPricing: selectedPricing ?? primaryPricing,
			primaryPricing,
		};
	});
